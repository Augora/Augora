...
export default class MapCirco extends Component<IDistrict> {
  mapRef: any
  map: any
  selectedCirco: object

  constructor(props: IDistrict) {
    super(props)
  }
    ...
    this.selectedCirco = GEOJsonCirco.features.find(circo => {
      return (
        circo.properties.nom_dpt.toLowerCase() ===
          retirerAccentsFR(this.props.nom.toLowerCase()) &&
        parseInt(circo.properties.num_circ) === this.props.num
      )
    })
  ...
  render = () => {
    return (
      <Block>
        <p>{this.props.nom}</p>
        <MapBlock ref={this.mapRef} className="map"></MapBlock>
      </Block>
    )
  }
}
